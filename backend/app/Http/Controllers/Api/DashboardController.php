...
class DashboardController extends Controller
{
    public function stats(Request $request)
    {
        ...
        return Cache::remember($cacheKey, 600, function() use ($user) {
            ...
            if ($user->role === 'admin') {
                ...
                $certificatesCount = \App\Models\Certificate::where('student_id', $studentId)->count();
                
                // Attendance rate
                $totalAttendances = \App\Models\Attendance::where('student_id', $studentId)->count();
                $attendanceRate = $enrolledCourses > 0 ? min(100, $totalAttendances * 10) . '%' : '0%';
    ...
    public function reports(Request $request)
    {
        ...
        return Cache::remember('admin_reports', 1800, function() {
            return [
                'users' => [
                    'total' => User::count(),
